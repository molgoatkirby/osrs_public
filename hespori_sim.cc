...
int dharok(int atk, int def, int max, double hpCur, double hpMax) {
	int temp = hit(atk, def, max);
	double multiplier = 1 + ((hpMax - hpCur)/100) * hpMax / 100;
...
struct npcBoss {
	...
	//stats
	int baseHp  = 300;	int curHp  = baseHp;
	int baseAtk = 1;	int curAtk = baseAtk;
	int baseStr = 1;	int curStr = baseStr;
	int baseDef = 120;	int curDef = baseDef;
	int baseMag = 126;	int curMag = baseMag;
	int baseRng = 150;	int curRng = baseRng;

	//equipment offence bonuses
	int equipMeleeAtk = 0;
	int equipMagicAtk = 150;
	int equipRangeAtk = 150;

	//equipment defence bonuses
	int equipStabDef  = 60;
	int equipSlashDef = 20;
	int equipCrushDef = 60;
	int equipMagicDef = 80;
	int equipRangeDef = 80;
...
npcBoss fightBoss(gear myGear){
	...
	boss.clawBoss(myGear.accClaws, myGear.maxClaws);
	boss.scytheBoss(myGear.accScythe, myGear.maxScythe); 
	boss.scytheBoss(myGear.accScythe, myGear.maxScythe); 
	boss.scytheBoss(myGear.accScythe, myGear.maxScythe); 
	boss.scytheBoss(myGear.accScythe, myGear.maxScythe); 
	boss.thrallHit();
	...
	//method 2
	boss.clawBoss(myGear.accClaws, myGear.maxClaws);
	boss.challyBoss(myGear.accChally, myGear.maxChally);
	boss.scytheBoss(myGear.accScythe, myGear.maxScythe);
	boss.scytheBoss(myGear.accScythe, myGear.maxScythe);
	boss.scytheBoss(myGear.accScythe, myGear.maxScythe);
	boss.challyBoss(myGear.accChally, myGear.maxChally);
	...
	*/
	...
	boss.ddsBoss(myGear.accDDS, myGear.maxDDS);
	boss.challyBoss(myGear.accChally, myGear.maxChally);
	boss.challyBoss(myGear.accChally, myGear.maxChally);
	boss.thrallHit();
	*/
...
int main() {
	...
	int scytheA  = 147;		int scytheMax  = 48;
	int clawsA   = 94; 		int clawsMax   = 43;
	int bgsA     = 0;		int bgsMax     = 0;
	int ddsA     = 107;		int ddsMax     = 41;
	int challyA  = 147;		int challyMax  = 58;
	int dharokA  = 140;		int dharokMax  = 52;
